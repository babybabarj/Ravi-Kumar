# Strategy Report Card: `STRAT_A2_1_TREND_FAST`

**Status**: ❌ REJECTED  
**Family**: `FAMILY_A2_MULTI_TF_TREND`  
**Symbol**: `BTCUSDT`  
**Evaluation Timestamp**: `2026-09-20T21:32:47.247335+00:00`  
**Code Commit**: `e0a94eb22b92d823849c041627b87bedaf06e347`  
**Dataset SHA-256**: `59af811657786156...`

---

## 1. Core Hypothesis
> Multi-day trend continuation on 1h bars when 48h trend slope confirms and funding rate is non-extreme.

**Parameters**:
```json
{
  "fast": 24,
  "slow": 96,
  "slope_thresh": 0.005,
  "strat_id": "STRAT_A2_1_TREND_FAST"
}
```

---

## 2. Empirical Performance Summary (Out-of-Sample)

| Metric | Measured Value | Validation Threshold | Status |
| :--- | :--- | :--- | :--- |
| **OOS Trades** | 679 | $\ge 30$ | PASS |
| **Gross Return** | -1.72% | N/A | INFO |
| **Net Return (Base Costs)** | -69.62% | $> 0.00\%$ | FAIL |
| **Net Return (Stressed Costs)** | -96.05% | $> 0.00\%$ | FAIL |
| **Annualized Sharpe** | -3.66 | $\ge 1.00$ | FAIL |
| **Annualized Sortino** | -5.63 | $\ge 1.30$ | FAIL |
| **Max Drawdown** | 75.13% | $\le 15.00\%$ | FAIL |
| **Profit Factor** | 0.61 | $\ge 1.25$ | FAIL |
| **Win Rate** | 16.5% | N/A | INFO |
| **Net Expectancy (bps/trade)**| -16.6 bps | $\ge +2.0$ bps | FAIL |

---

## 3. Statistical Rigor & Overfitting Diagnostics

| Test / Diagnostic | Result | Acceptance Limit | Outcome |
| :--- | :--- | :--- | :--- |
| **Deflated Sharpe Ratio (DSR)** | 0.000 | $\ge 0.95$ ($p < 0.05$) | FAIL |
| **Probability of Backtest Overfit (PBO)** | 0.00 | $\le 0.50$ | PASS |
| **Parameter Neighborhood Stability** | 50.0% | $\ge 60.0\%$ | FAIL |
| **Top 1% Trade Concentration** | 18.0% | $\le 35.0\%$ | PASS |
| **Bootstrap Expectancy 95% Lower CI** | +0.0 bps | $\ge 0.0$ bps | PASS |

---

## 4. Promotion Gate Findings & Decision

**Final Tier**: `REJECTED`

### Recorded Findings / Deficiencies:
- Negative net return after base costs (-0.6962 <= 0)
- Fails cost stress survival: net return under stressed costs is -0.9605 <= 0
- Max drawdown exceeded limit (0.751 > 0.150)
- Sharpe ratio below threshold (-3.66 < 1.00)
- Parameter instability (50.0% profitable neighbors < 60.0%)
- Multi-year inconsistency: profitable years 0.0% < 66.0% or max single-year drawdown 79.5% > 12.0%
- Deflated Sharpe Ratio failed multiple testing penalty (DSR=0.000 < 0.95)

---
*Report generated automatically by BTCETH Trading OS Market Brain Research Engine.*
