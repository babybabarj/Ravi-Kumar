...
<script>const esc=v=>v===null||v===undefined?'—':String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const cls=s=>/READY|HEALTHY|ACCEPTED|FILLED|CONNECTED/.test(s)?'good':/DEGRADED|GAP|FAILED|STALE|REJECTED/.test(s)?'bad':'warn';function card(title,status,rows){return `<section class="card"><div class="label">${title}</div><div class="value ${cls(status)}">${esc(status)}</div>${rows.map(([k,v])=>`<div class="row"><span>${esc(k)}</span><strong>${esc(v)}</strong></div>`).join('')}</section>`}async function refresh(){try{let s=await fetch('/api/state',{cache:'no-store'}).then(r=>r.json()),a=s.account;cards.innerHTML=card('Data health',s.data_health.status,[['Last event',s.data_health.last_event_ns],['Open gaps',s.data_health.open_gaps]])+card('Market state',s.market.status,[['Instrument',s.market.instrument],['Price',s.market.price]])+card('Signal',s.signal.status,[['Target position',s.signal.target_position],['Reason',s.signal.reason]])+card('Simulated position',s.paper.status,[['Cash',s.paper.cash],['Position',s.paper.position],['Equity',s.paper.equity],['P&L',s.paper.pnl]])+card('Binance account',a.status,[['Spot balances',a.spot.balances.length],['Spot open orders',a.spot.open_orders],['USD-M positions',a.usdm.positions.length],['USD-M open orders',a.usdm.open_orders],['Last sync',a.last_sync_ns],['Status detail',a.error]]);updated.textContent='Updated '+new Date().toLocaleTimeString()}catch(e){updated.textContent='Dashboard state unavailable'}}refresh();setInterval(refresh,2000)</script></body></html>"""
